...
**Orient, don't explore.** Read the project's patterns record, if it has one
(`docs/patterns.md`), then `primer.md`, the issue, and
...
**Precedence, where the project keeps a patterns record: the patterns record
...
**Then attack it.** Invoke the `code-review` skill on the issue's diff if the setup
registers one, otherwise read the diff yourself. Beyond it, ask the
...
patterns record wins where the project keeps one: if it endorses a shape this
list would flag, cite the entry and suppress the smell. And every item is a judgement call
...
mutant instead of the real file: both happened on one issue, where the tree sat
reverted mid-gate (skills/run-issues/decisions.md, "Post-run revisions
(2026-08-02)"). Recover a suspect file from
git or from a built artefact, never from your own mid-run backup. Record each graded file's
